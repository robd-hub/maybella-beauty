...
Payment details (added Sept 2026, real — from a "Mode of Payment" graphic Rob supplied directly from May): contact.html's qr-card now lists real account details for GCash (09561845949), BDO (011270071252), PNB (228810267198) and PayNow/Singapore (82004552), all under "May S. Balabbo", plus a note to send proof of payment via Facebook. Styled as `.payment-methods` in style.css, below the QR box. Note: the source graphic also listed two Facebook pages ("Maybella Beauty by Bossed Up Gals" and "Maybella Signature") and an Instagram handle @maybellabeautyph, which don't exactly match the Facebook/Instagram links already on the site (facebook.com/profile.php?id=100089975888895, @maybellabeauty) — left the existing site links alone since this wasn't confirmed as a correction, just flagging the discrepancy for Rob to check with May.

Real GCash QR code (added Sept 2026, assets/gcash-qr.png): the `.qr-box` placeholder is now May's actual InstaPay-linked GCash QR (mobile ending 0956 184, name shown as "M** B." per GCash's own masking). Cropped from a screenshot Rob AirDropped to himself (landed in ~/Downloads, not pasted inline, since chat-pasted images aren't saved to a reachable file — redrawing a payment QR from a screen capture wasn't attempted, it was tracked down as a real file instead). `.qr-box` CSS changed from a dashed-border placeholder div to a plain bordered/shadowed image frame (200px, object rendered as an `<img>` now, not a `<div>`).

Invented / placeholder — flag to Rob before treating as final:

FAQ payment answer: original site's FAQ said "Visa, Mastercard, PayPal via checkout" (leftover Shopify boilerplate that doesn't reflect reality). Rewritten here to say GCash + bank transfer via DM, matching how May actually operates — now corroborated by the real payment methods above. If May starts using a real checkout later, this needs to change back.
