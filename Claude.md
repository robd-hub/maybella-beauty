...
--rose (#ca4043) / --rose-dark (#a43235) / --rose-light (#f5cfd0) / --rose-pale (#fbf1f1) — primary brand accent (rose = action/interactive: buttons, links, prices). Went through two rounds Sept 2026: first changed from magenta-pink (#c04f67) to a coral/terracotta (#d25042) to echo the hero photo's coral backdrop, then Rob supplied a specific dusty-rose swatch (#e49ea0) to use instead — but like the photo color before it, that swatch only gives 2.17:1 contrast with white button text (fails WCAG's 4.5:1). Landed on the current value: the swatch's exact hue/saturation at a deeper lightness (4.85:1, legible). --border/--border-alt and --cream-dark stay derived from whatever --rose is, so they moved too, keeping card borders and alt-bg sections cohesive.
