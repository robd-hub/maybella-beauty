...
GCash QR box (contact.html, .qr-box): still a dashed placeholder that says "Your GCash QR code goes here" — May sent a real GCash QR image (InstaPay, her name) but as a pasted screenshot with no source file reachable on disk, so it wasn't wired in (redrawing a payment QR from a screen capture risks encoding the wrong data). Replace with the actual image file once it can be saved into assets/gcash-qr.png.
